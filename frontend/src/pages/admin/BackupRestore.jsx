...
export default function BackupRestore() {
  ...
  const handleDownloadMdb = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `MRS_SOLAR_Database_Backup_${Date.now()}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch (error) {
      console.error('Download Excel backup failed:', error);
      alert('Failed to download Excel database backup.');
    }
  };
